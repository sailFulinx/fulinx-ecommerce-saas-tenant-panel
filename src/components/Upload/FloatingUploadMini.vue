...
<template>
  <div class="relative">
    <div v-if="loading" v-loading="loading" class="w-10 h-10 border border-dashed border-gray-200" />
    <div v-else>
      <!-- 上传按钮 -->
      <div
        v-if="showUploadButton"
        class="w-10 h-10 bg-white flex items-center group cursor-pointer relative"
        @mousedown="showHoverUpload"
        @mouseleave="hideHoverUpload"
      >
        <div class="flex items-center transition-colors duration-300 group-hover:text-[#71A0FF]">
          <Icon :size="6" icon="ep:picture" color="#999" />
        </div>
      </div>
      ...
      >
        ...
        <FileList ref="fileListRef" :accept-file-type="acceptFileType" :max-selectable-count="maxCount" @selection-confirmed="setSelectionFileData" />
      </div>
    </div>
  </div>
</template>
