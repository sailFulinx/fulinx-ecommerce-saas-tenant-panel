...
const handleRedirectEdit = (val: CouponListData & CommonField) => {
  router.push({ name: 'ShowCoupon', params: { id: val.id } })
}
init()

const handleCreate = () => {
  router.push({ name: 'CreateCoupon' })
}
...
const handleDelete = async (row: CouponListData & CommonField) => {
  loading.list = true
  await removeCouponApi({ ids: [row.id] }).catch(err => {
    loading.list = false
    throw err
  })
  getList()
...
const handleMultiDelete = async () => {
  ...
  await removeCouponApi({ ids: selectedList.value }).catch(err => {
    loading.list = false
    throw err
  })
  loading.list = false
  ...
  <div class="view-page">
    ...
    <div class="view-main">
      <ElTable
        ...
        border
        @selection-change="selectedCouponItem"
      >
        <ElTableColumn type="selection" width="55" />
        <ElTableColumn :label="$t('coupon.couponName')">
        ...
        <ElTableColumn label="操作" header-align="center" width="220" align="center" class-name="pl-15 fixed-width">
          <template #default="scope">
            <span class="mr-5">
              ...
              <EBtn size="small" @click="handleDelete(scope.row)">
                <Icon icon="ep:delete" class="mr-1" />
                {{ $t('common.delete') }}
              </EBtn>
            </span>
