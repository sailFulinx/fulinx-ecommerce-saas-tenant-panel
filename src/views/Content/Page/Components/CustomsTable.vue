...
const updatePageCustom = async () => {
  if (!props.pageDetailId) {
    return
  }
  const payload = {
    pageDetailId: props.pageDetailId,
    customs: JSON.stringify(customs.value),
  }
  await updatePageCustomsApi(payload).catch((error: any) => {
    throw error
  })
