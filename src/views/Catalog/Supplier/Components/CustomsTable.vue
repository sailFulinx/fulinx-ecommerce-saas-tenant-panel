...
const updateSupplierCustom = async () => {
  if (!props.supplierDetailId) {
    return
  }
  const payload = {
    supplierDetailId: props.supplierDetailId,
    customs: JSON.stringify(customs.value),
  }
  await updateSupplierDetailCustomApi(payload).catch((error: any) => {
    throw error
  })
...
const getExistingFieldNames = () => {
  ...
  return customs.value.map(item => item.customFieldName)
}
