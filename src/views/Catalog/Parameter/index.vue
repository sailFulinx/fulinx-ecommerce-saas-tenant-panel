...
import CreateParameterDialog from './Components/CreateParameterDialog.vue'
...
const createParameterRef = useTemplateRef('createParameterRef')
...
// 处理排序变更
const handleSortChange = async (row: ParameterListData & CommonField, value: number | undefined) => {
...
const handleUpdateParameterStatus = async (row: ParameterListData & CommonField, value: boolean) => {
  loading.list = true
  await updateParameterStatusApi({
    parameterId: row.id,
    languageId: languageId.value,
    status: value,
  })
        ...
        <ElTableColumn :label="$t('common.status')" width="120">
          <template #default="scope">
            <ElSwitch
              ...
              @change="handleUpdateParameterStatus(scope.row, Boolean($event))"
            />
          </template>
        </ElTableColumn>
