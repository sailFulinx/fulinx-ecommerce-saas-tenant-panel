...
const updateBrandCustom = async () => {
  if (!props.brandDetailId) {
    return
  }
  const payload = {
    brandDetailId: props.brandDetailId,
    customs: JSON.stringify(customs.value),
  }
  await updateBrandDetailCustomApi(payload).catch((error: any) => {
    throw error
  })
