...
const { loading, resetFormData } = inject(brandKey)!
...
const handleEditBrandLayout = async () => {
  isShowLayoutEdit.value = true
  await nextTick()
  if (
    currentItem
    && currentItem.brandDetailListResultDo?.layoutContent
    && currentItem.brandDetailListResultDo?.layoutType === 3
    && simplifiedComponentLayoutRef.value
  ) {
    rows.value = JSON.parse(currentItem.brandDetailListResultDo.layoutContent)
...
watch(
  () => currentItem,
  () => {
    if (currentItem.brandDetailListResultDo?.layoutType) {
      currentLayoutType.value = currentItem.brandDetailListResultDo.layoutType
      isShowLayoutEdit.value = true
    }

    if (currentItem.brandDetailListResultDo?.devComponentName) {
      devComponentName.value = currentItem.brandDetailListResultDo.devComponentName
    }
...
const handleSubmitBrandLayout = async (val: BrandAdminLocalizedViewDo) => {
  loading.init = true
  if (val.brandDetailListResultDo === null) {
    return
  }
          ...
          <div class="col-span-11">
            <div v-if="!isShowLayoutEdit">
              <div class="flex justify-start items-center">
                <span class="mr-2">
                  {{ currentItem.brandDetailListResultDo?.layoutTypeLabel }}
                </span>
                <span>
                  <ElButton type="primary" text @click="handleEditBrandLayout">
              ...
              <div class="w-full flex items-center justify-between mb-5">
                ...
                <div>
                  <div class="w-full flex">
                    ...
                    <ElButton type="danger" @click="handleSubmitBrandLayout(currentItem)">
      ...
      <div v-if="currentLayoutType === 3" class="border border-gray-200" :class="{ 'full-screen-mode': isFullScreen }">
        ...
        >
          ...
          <div v-if="isFullScreen" class="flex justify-end space-x-2">
            ...
            <ElButton type="primary" @click="handleSubmitBrandLayout(currentItem)">
