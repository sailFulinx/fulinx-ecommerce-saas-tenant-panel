...
import { createArticleBaseApi, fetchArticleTypeListApi, showArticleApi } from '@/api/article'
...
const handleRemoveCategory = (val: number[]) => {
  console.log(val)
  articleForm.value.deletedCategoryIds?.push(val.pop() as number)
...
const save = async () => {
  articleForm.value.categoryIds = [...new Set(selectedCategoryValue.value.flat() as number[])]
  ...
  //   })
  // }
  // console.log(articleForm.value.categoryIds)
