...
const listData = ref<TableResponse<LayoutData & CommonField>>({
  list: [],
  total: 0,
})
...
const getList = debounce(async () => {
  loading.value = true
  if (listPayload.layoutName === '') {
    listPayload.layoutName = null
  }
  if (listPayload.layoutName && listPayload.layoutName?.length <= 1) {
    loading.value = false
    return
  }
  const { data } = await layoutPaginationApi(listPayload).catch(error => {
    loading.value = false
    throw error
  })
  listData.value = data
...
const handleDelete = async () => {
  ...
  loading.value = true
  await removeLayoutApi({ ids: selection.value }).catch(error => {
    loading.value = false
    throw error
  })
  getList()
    ...
    <div v-loading="loading" class="w-full overflow-hidden" :style="{ height: `${screenHeight}px` }">
      <div class="grid grid-cols-12 gap-5">
        <div class="col-span-3 lg:col-span-3 xl:col-span-3 2xl:col-span-3 bg-white pa-5 border-r border-gray-200" :style="{ height: `${screenHeight}px` }">
          ...
          <div class="w-full">
            <ElInput v-model="listPayload.layoutName" clearable placeholder="请输入关键字搜索" @input="getList" />
          </div>
          <div class="pa-0 mt-5">
            <ElTreeV2
              ref="treeRef"
